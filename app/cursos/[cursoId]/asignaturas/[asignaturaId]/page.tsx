...
export default function AsignaturaPage() {
  ...
  useEffect(() => {
    const asignaturasCurso = snap[cursoId]
    if (asignaturasCurso) {
      const encontrada = asignaturasCurso.find((a) => a.id === asignaturaId)
      if (encontrada) setAsignatura(encontrada)
    }
  }, [cursoId, asignaturaId, snap])
          ...
          {asignatura.ra.map((ra: any, index: number) => (
            <Card key={index} className="border-muted">
              <CardContent className="p-4">
                <h2 className="font-semibold text-lg">{ra.codigo} – {ra.descripcion}</h2>
